...
out = cv2.VideoWriter(scene_source, fourcc, 20.0, (640, 480))

################################################################################################
#  10) Robotic Arm movement detection
################################################################################################
motion_buffer = deque(maxlen=MAX_FRAMES)
...
def safe3dzone_thread():
    ...
    try:
        ...
        ROBOT_MOVING = False                        # init. detect robot moving state
        last_positions = {}
        MOVE_THRESHOLD = 5
        ...
        while True:
            ...
            # Marker Detection for Robot Movement Detection
            ...
            for box in marker_results[0].boxes:
                ...
                cls = int(box.cls[0])
                #if label == "marker" and conf > 0.5:    # confidence output is rather low. so, lower the threshold
            ...
            # Marker Tracking for Improved marker Detection
            deepsort_dets = []
            if det != None:
                w = det[2][2] - det[2][0]
                h = det[2][3] - det[2][1]
            ...
            for t in tracks:
                ...
                #     hist_box = np.mean(np.array(motion_buffer),axis=0)
                #     mae = np.mean(np.abs(hist_box - new_box))
                #     #print(f"{len(motion_buffer)}:{np.mean(np.array(motion_buffer),axis=0)}:{mae}")

                #     if mae >= 10 and ROBOT_MOVING == False: #motion threshold
                #         ROBOT_MOVING=True
                #         start_moving = datetime.now(timezone.utc)
                #         now_kst = start_moving.astimezone(ZoneInfo("Asia/Seoul")).strftime("%Y-%m-%d %H:%M:%S")
                #         print(f"{now_kst} Robot starts moving")

                #     if mae < 10 and ROBOT_MOVING == True:
                #         end_moving = datetime.now(timezone.utc)
                #         now_kst = end_moving.astimezone(ZoneInfo("Asia/Seoul")).strftime("%Y-%m-%d %H:%M:%S")
                #         time_diff = start_moving - end_moving
                #         time_diff = time_diff.total_seconds()
                #         if time_diff > 10:
                #            ROBOT_MOVING=False 
                #         print(f"Robot stopped moving at {now_kst} after {time_diff}-sec operation")
                # motion_buffer.append(new_box)
                ...
                if obj_id in last_positions:
                    ...
                    moving = False
                #cv2.putText(color_image, f"Moving: {moving}", (x1, y1-25),cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,0,0), 2)
                last_positions[obj_id] = (cx, cy)

                motion_buffer.append(moving)
                if np.sum(motion_buffer) > 2:
                    ROBOT_MOVING=True
                else:
                    ROBOT_MOVING=False
                cv2.putText(color_image, f"Robot Moving: {ROBOT_MOVING}", (x1, y1-30),cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,0), 2)

            # yolov8 detection on persons 
            ...
            # Inserting Image into Shared Memory to pass it to Gstreamer thread 
            with lock:
            ...
            if violate == True:                                     # 
                vio_cnt += 1
                # if vio_cnt > MAX_VIOLATE and IN_VIO == False and ROBOT_MOVING == True:   
                if vio_cnt > MAX_VIOLATE and IN_VIO == False:       # check if or not to enter in-vion state
                    IN_VIO, novio_cnt = True, 0                     # mark check-in, clear no_vio_cnt
                    for frame in frame_buffer:                      # write all buffered frames 
                        print("Frame Buffer Writing...")
                        out.write(frame)
                    start_kst = datetime.now(timezone.utc).astimezone(ZoneInfo("Asia/Seoul")).strftime("%Y-%m-%d_%H-%M-%S")
                    print(f"{now_kst} Write video recording")
            
            if violate != True :
                novio_cnt +=1
                if novio_cnt > MAX_NONVIOLATE and IN_VIO == True:
                    IN_VIO, vio_cnt = False, 0
                    out.release()
                    stop_kst = datetime.now(timezone.utc).astimezone(ZoneInfo("Asia/Seoul")).strftime("%Y-%m-%d_%H-%M-%S")
                    print(f"Stop REC.:{stop_kst}")
                    print(f"Stop REC..:{start_kst}") 
                    scene_output = scene_destination + start_kst + "__" + stop_kst + ".avi"
