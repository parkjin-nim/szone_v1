...
def draw_red_geo_fence(color_image,depth_image,color1,p1,p2,p3,p4,p6,p7,p8,p9):
...
scene_source= './output.avi'
scene_destination = '/mnt/webdav/DGSP-ZONE-INFERENCE-TAPO1/'
fourcc = cv2.VideoWriter_fourcc(*'XVID')  # 비디오코덱, SVID. 'mp4v' for MP4
ZONE_STATE = 0
################################################################################################
...
def safe3dzone_thread():
    ...
    model = YOLO("./yolo11n.pt")     # 10.1.1 yolo11 setting, yaml file failure handling  
    #CLASSES = yaml_load('./coco128.yaml')['names']
    CLASSES = yaml_load('./coco128.yaml')     
    colors = np.random.uniform(0, 255, size=(len(CLASSES), 3))

    marker_model = YOLO("./yolo11n_custom.pt")# 10.1.2 yolo11 setting for Marker Tracking  
    tracker = DeepSort(max_age=60)
    
    # realsense sdk setting:  Configure depth and color streams
    ...
    pipeline.start(config)     # Start rs streaming
    print("Start realsense streaming pumping! ")
   
    try:
        #vio_cnt, novio_cnt, IN_VIO = 0, 0, False   # init. safe zone violation rule
        is_person_obs = False
        is_robot_moving = False                     # init. detect robot moving state
        motion_buffer = deque(maxlen=30)            # 10 sec if fps is 3
        last_positions = {}
        MOVE_THRESHOLD = 5
        ...
        while True:
            ...
            # Set s-points Here : p1: the closest ptr, p2: right, p3: left, p4: behind p1 
            #
            ...
            pp1[2],pp2[2],pp3[2],pp4[2],pp6[2],pp7[2],pp8[2],pp9[2] = \
                ...
                int(depth_image[pp1[1], pp1[0]]), int(depth_image[pp2[1], pp2[0]]), int(depth_image[pp3[1], pp3[0]]),int(depth_image[pp4[1], pp4[0]])
            p1,p2,p3,p4,p6,p7,p8,p9 = pp1[:2],pp2[:2],pp3[:2],pp4[:2],pp6[:2],pp7[:2],pp8[:2],pp9[:2]
            ...
            color1 = (0, 0, 255) #Red fence
            draw_red_geo_fence(color_image,depth_image,color1,p1,p2,p3,p4,p6,p7,p8,p9)
            # Apply colormap on depth image (image must be converted to 8-bit per pixel first)
            depth_colormap = cv2.applyColorMap(cv2.convertScaleAbs(depth_image, alpha=0.03), cv2.COLORMAP_JET)

            # Marker Detection Model.
            # DeepSort Marker Tracking Added.
            # Mark 'ROBOT_MOVING' flag, if robot moving
            ...
            for box in marker_results[0].boxes:
                ...
                cls = int(box.cls[0])
                if conf > conf_old:  # Get top-confidence 1 marker
                    conf_old = conf
                    det = ("marker", conf, (x1, y1, x2, y2)) 
            if det != None:
            ...
            if det != None:
                w = det[2][2] - det[2][0]
                h = det[2][3] - det[2][1]
                deepsort_dets.append([[det[2][0], det[2][1], w, h], det[1]])

            tracks = tracker.update_tracks(deepsort_dets, frame=color_image)
            for t in tracks:
                if not t.is_confirmed():
                    print(f"t.is_confirmed.{len(tracks)}")
                    continue
                ...
                motion_buffer.append(is_moving)
                sum_move = np.sum(motion_buffer)
                if sum_move > 2:
                    is_robot_moving=True
                else:
                    is_robot_moving=False
                cv2.putText(color_image, f"Robot Moving: {is_robot_moving}-{sum_move}", (x1, y1-30),cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,0), 2)
            
            # Person Detection Model  
            # On left-top, display KST Time, display FPS, is_person, ZONE_STATE
            # Processing Bboxes, Chekc 'is person observed'. 
            # if exist, count key-counts in the safe3dzone.
            # if half of key-counts fall into it. flag 'is person observed' 
            ...
            now_kst = current_time.astimezone(ZoneInfo("Asia/Seoul")).strftime("%Y-%m-%d %H:%M:%S")
            cv2.putText(color_image, f"{now_kst}", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 2)
            fps = 1.0 / (time.time() - start_time)
            cv2.putText(color_image, f"FPS:{fps:.1f} is_person:{is_person_obs}({ZONE_STATE})", (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 2)
            ...
            for i in range(len(bboxes)):
                ...
                if label == 'person' and confidences[i] > 0.5:
                    if i in result_boxes:
                        ...
                        if cropped.size == 0:
                            continue
                        cropped_rgb = cv2.cvtColor(cropped, cv2.COLOR_BGR2RGB)# Convert to RGB for MediaPipe
                        results_mp = pose.process(cropped) # (MediaPipe requires RGB input)
                        key_list = []
                        if results_mp.pose_landmarks:
                            visible_landmarks = [                     
                                (idx, lm)
                                for idx, lm in enumerate(results_mp.pose_landmarks.landmark)
                                if lm.visibility > VISIBILITY_THRESHOLD # Get only visible keypoints
                            ]
                        ...
                        cv2.rectangle(color_image, (x1, y1), (x2, y2), (255, 0, 0), 2) # Draw person box
                        ...
                        for key in key_list:
                            ...
                            if  a1*cx + b1*cy + c1*cz + d1 > 0:
                                ...
                                key_str+="1"
                        if key_cnt > len(key_list) // 2:   # How many keypoints are there for him                         
                            is_person_obs = True # Violation rule setting: For at least one person, Mark person_observed flag 
                            log.info(f"[{key_cnt}/{len(key_list)}] VIOLATION")
                    ...
                    pass
                    #print(f"Detected {label} ignored.")

            # On screen display window. Resize color image to match depth image for display
            # Write image on shared memory for gstreamer's thread access
            # Producer's publishing on the topic
            #
            ...
            with lock:                          # Inserting Image into Shared Memory to pass it to Gstreamer thread 
                shared_a[:] = color_image
            GLib.idle_add(streaming_loop.quit)  # Schedule GLib loop to stop after work is done
            cv2.waitKey(1)
            producer.send(TOPIC_PUBLISH, {"timestamp": now_kst,"state": is_person_obs})

            # Check violation rule using state machine. 
            # (STATE) 0: No one, 1: One exist, 2: Waiting
            if ZONE_STATE == 0: # Zone is Empty   
                if is_person_obs:     
                    video = cv2.VideoWriter(scene_source, fourcc, 10.0, (640, 480)) # 파일이름, 비디오코덱, fps, 프레임크기
                    ...
                    pass
            elif ZONE_STATE == 1: # Zone is Full
                video.write(color_image)
                print(f".",end="")
                if not is_person_obs:     
                    ZONE_STATE = 2
                else:
                    pass
            elif ZONE_STATE == 2: # Waiting after Full Zone
                video.write(color_image)
                print(f".",end="")
                if is_person_obs:     
                    ZONE_STATE = 1
                else:
                    if  current_time - rec_start_time > timedelta(seconds=5):
                        start_kst = rec_start_time.astimezone(ZoneInfo("Asia/Seoul")).strftime("%Y-%m-%d %H:%M:%S")
                        scene_output = scene_destination + start_kst + "__" + now_kst + ".avi" #save video file
                        shutil.copy(scene_source, scene_output)
                        print()
                        print(f"End recording at {scene_output}")
                        video.release()
                        ZONE_STATE = 0
                    else:
                        pass
...
class CustomMediaFactory(GstRtspServer.RTSPMediaFactory):
    def __init__(self, **properties):
        ...
        self.launch_string = (
            ...
            #"appsrc name=mysrc is-live=true block=true format=TIME caps=video/x-raw,format=BGR,width=640,height=480,framerate=10/1 ! "
            "videoconvert ! "
